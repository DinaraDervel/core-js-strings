...
function formatTime(minutes, seconds) {
  let min = '';
  let sec = '';
  if (minutes < 10) min = minutes.toString().padStart(2, '0');
  else min = minutes;
  if (seconds < 10) sec = seconds.toString().padStart(2, '0');
  else sec = seconds;
...
function countVowels(str) {
  const arr = str.match(/a|e|i|o|u|y/gi);
  if (!str || !arr) return 0;
  return arr.length;
}
...
function getCardId(value) {
  ...
  if (suitIndex === -1 || rankIndex === -1) throw new Error('Invalid card');
